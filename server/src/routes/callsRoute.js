...
router.get("/my-calls", authMiddleware, async (req, res) => {
  try {
    ...
    // Auto-link calls by agent_name
    // This is a fallback when webhooks haven't linked calls automatically
    // Also checks for new calls even if some are already linked
    try {
      ...
      // Also query Retell API directly for calls with matching agent_name to find any missing calls
      // This ensures we catch calls that don't have CallRecords yet
      try {
        ...
        for (const call of matchingCalls) {
          try {
            ...
            if (!created && !callRecord.userId) {
              ...
              await callRecord.save();
              linkedCount++;
              console.log(`✅ Linked existing call ${callId} to user ${user.username}`);
            } else if (created) {
              linkedCount++;
